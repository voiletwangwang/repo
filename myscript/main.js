...
var board = new Array();
var score = 0;
var hasConflicted = new Array();//用来判断每个格子是否已经发生碰撞
...
function prepareForMobile(){
    ...
    $('#grid-container').css('width',gridContainerWith - 2*cellSpace);
    $('#grid-container').css('height',gridContainerWith - 2*cellSpace);
    $('#grid-container').css('padding',cellSpace);
    $('#grid-container').css('border-radius',0.02*gridContainerWith);
...
* 3、初始化数据 清零 updateBoardView()Wangyanq!n
*/
function init() {
    for (var i = 0; i < 4; i++){
        for(var j = 0; j < 4; j++){
...
function updateBoardView() {
    ...
    $('.number-cell').css('font-size',0.6*cellSideLength+'px')//注意这里之前是0.6.现在文字版是0.3
